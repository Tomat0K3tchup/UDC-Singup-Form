    ...
    for (let i = 0; i < zf_MandArray.length; i++) {
        ...
        if (fieldObj) {
            if (fieldObj.nodeName === 'OBJECT') {
                ...
                zf_ShowErrorMsg(fieldName);
                return false;
            } else if (fieldObj.length) { // Handle multi-selects or groups like checkboxes/radios
...
function zf_ValidatePhone(elem) {
    ...
    if (phoneFormat === 1) {
        if (elem.getAttribute("valType") === 'code') {
            ...
            const regex = elem.getAttribute("phoneFormatType") === '2' ? ZFPhoneRegex.PHONE_INTE_NUMERIC_REG : ZFPhoneRegex.PHONE_INTE_ALL_REG;
            return regex.test(fieldInpVal);
        }
    } else if (phoneFormat === 2) {
        const maxLength = elem.getAttribute("maxlength");
        return fieldInpVal === "" || (ZFPhoneRegex.PHONE_USA_REG.test(fieldInpVal) && fieldInpVal.length === maxLength);
    }
    return true;
}
